...
```{r}
ynull =-11; ymax =-8;xmin = -0.5; xmax = .25;
singleboundary.plot <- ggplot2::ggplot(data = plot.data.dpsiX[plot.data.dpsiX$N==8,])+
  ...
  theme_linedraw()+coord_fixed(ratio = (xmax -xmin)/(ymax-ynull))
singleboundary.plot+
...
#plot
medium.colours <- setNames(c("darkgreen", "lightgreen", colorRampPalette(c("yellow", "orange"))(6)), unique(plot.data$Medium))
boundary.plots + 
  geom_point(aes(x = dpsi.scaled, y = loggamma.scaled, colour = Medium, shape = Plasmid), data = plot.data, size =1.5, alpha =.5)+
  scale_colour_manual(values = medium.colours, name = "Growth medium")+
  scale_shape_manual(values = c(15,17), name = "Plasmid carrying strain")+
  xlab(c(expression(paste(Delta, psi))))+
