---
title: "R-code with 'Microbiome-derived lactic acid bacteria increase likelihood of antimicrobial resistance plasmid loss in Escherichia coli populations'"
...
## R functions
```{r echo=F}
#functions describing the boundary between trivial and coexistence and between coexistince and fixation. 
...
#conjugation data (now in one file previously separated files. still load twice for script to work)
conjugation.dataD <- read.csv(paste0(input.dir, "ConjugRate_files_scripts/20201027_Figure1c_conjug_rates_huisman_4h.csv"))
conjugation.dataT <- read.csv(paste0(input.dir, "ConjugRate_files_scripts/20201027_Figure1c_conjug_rates_huisman_4h.csv"))
stop("Data problem is not resolved.")
#load growth data 
growth.data <- read.csv(paste0(input.dir,"GrowthRate_files_scripts/20201027_Growth_rates_D_R_T.csv"))

#renaming to fit the existing script after renaming of columns
colnames(conjugation.dataT)[colnames(conjugation.dataT) %in% c("Treatment_group","Treatment", "Medium")] <- c("LABgenus","VL_type", "Experiment")
colnames(conjugation.dataD)[colnames(conjugation.dataD) %in% c("Treatment_group","Treatment", "Medium")] <- c("LABgenus","VL_type", "Experiment")
colnames(growth.data)[colnames(growth.data) %in% c( "Treatment_group","Treatment")] <- c("LABgenus","VL_type")
...
data.combinations$dpsiDR.scaled  <- (data.combinations$D -data.combinations$R)/data.combinations$R
...
data.combinations<- data.combinations[order(data.combinations$Medium,
                                            data.combinations$Replicate),]
conjugation.dataD<- conjugation.dataD[order(conjugation.dataD$Experiment,
                                            conjugation.dataD$Replicate),]
conjugation.dataT<- conjugation.dataT[order(conjugation.dataT$Experiment,
                                            conjugation.dataT$Replicate),]
...
boundary.plots + 
  geom_point(aes(x = dpsi.scaled, y = loggamma.scaled, shape = Medium,colour = Medium), data = plot.data[plot.data$Plasmid=="D",], size =1.75, alpha =.75)+
...
boundary.plots + 
  geom_point(aes(x = dpsi.scaled, y = loggamma.scaled, colour = LABgenus), data = plot.data[plot.data$Plasmid=="T",], size =1.75, alpha =.75)+
  scale_colour_manual(values = labgenus.colours, name = "Medium")+
  #scale_shape_manual(values = labgenus.shapes, name = "LAB genus")+
   xlab(expression(paste("Growth rate difference: ", Delta, psi)))+
  ylab(expression(paste("log conjugation rate: ",  log[10](gamma))))+facet_grid(.~10^N)+
...
ggsave(paste0("C:/Surfdrive/Projecten/MITAR/ProjectShareMITAR/Papers/Microb_communities_conjugation/Figure_estimatesandequilibrium_GENUS_donor_samemarkers",format(Sys.Date(),"%Y%m%d"),".tiff"))
