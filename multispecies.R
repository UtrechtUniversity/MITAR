              ...
              for(taxmattype in taxmattypeset) {
                ...
                if(simulateinvasion == TRUE) {
                  if(eqinfoconj["eigvalRe"] >= 0 || saveplotconjovertime) {
                    ...
                    # new equilibrium.
                    abunfinalconj <- list(R = abundance,
...
colnames(plotdata) <- c("nsimul", "nspecies", "abunmodelcode", "intmean",
                        "selfintmean", colnames(summarydata))
...
if(simulateinvasion == TRUE) {
  ...
  if(PReplMostAbun == TRUE) {
    ...
    add_filltitleconj <- paste("after\nreplacing some R of the",
...
plotcompareabun <- ggplot(data = compareabun,
  ...
  theme(legend.box = "horizontal", legend.box.just = "left",
        legend.box.margin = margin(-5, 0, 0, 0), legend.justification = "left",
        legend.margin = margin(-5, 0, -5, 0), legend.position = "bottom",
...
plotcompareabunlog <- plotcompareabun +
  scale_y_continuous(breaks = 10^seq(from = floor(log10(min(compareabun$abun))),
                                     to = ceiling(log10(max(compareabun$abun))),
                                     by = 2),
                     limits = c(10^floor(log10(min(compareabun$abun))), NA),
                     trans = "log10") +
