...
# to and from the initially plasmid-bearing population (here the least abundant
            ...
            for(taxmattype in taxmattypeset) {
              ...
              if(simulateinvasion == TRUE) {
                if(eqinfoconj["eigvalRe"] >= 0) {
                  pertpopadjconj <- paste0("P", nspecies)
                  abunfinalconj <- perturbequilibrium(abundance = c(abundance, rep(0, nspecies)),
                                                      intmat = intmat, growthrate = growthrate,
                                                      cost = cost, conjmat = conjmat,
                                                      model = "gLVConj", pertpop = pertpopadjconj, tmax = 1e4,
                                                      showplot = FALSE, verbose = FALSE,
                                                      suppresswarninfgrowth = TRUE)
                } else {
                  # No need for simulations if equilibrium is stable
...
DateTimeStamp <- format(Sys.time(), format = "%Y_%m_%d_%H_%M")
DateTimeStamp <- paste0(DateTimeStamp, "PInLeastAbund")
...
if(simulateinvasion == TRUE) {
  ...
  CreatePlot(fillvar = "log10(abunRsp4median)",
             filltitle = paste("Log10(Median abundance sp4 after\nperturbation",
                               "with R of least\nabundant species)"),
             filltype = "continuous")
  ...
  CreatePlot(fillvar = "log10(abunRsp6median)",
             filltitle = paste("Log10(Median abundance sp6 after\nperturbation",
                               "with R of least\nabundant species)"),
             filltype = "continuous")
